...
import {
  ...
  useStockValidationPopup,
} from "@/components/ui/stock-validation-popup";
...
export function CartItems({
  ...
  const { isOpen, validationResult, showValidation, hideValidation } =
    useStockValidationPopup();
  ...
  const pendingUpdates = useRef<
    Record<string, { quantity: number; sequence: number } | null>
  >({});
    ...
    setLocalQuantities((prev) => {
      ...
      items.forEach((item) => {
        ...
        if (pendingUpdates.current[menuItemId] === null) {
          ...
          pendingUpdates.current[menuItemId]?.quantity === item.quantity
        ) {
  ...
  const applyQuantityChange = (
    ...
    if (
      pendingUpdates.current[menuItemId] &&
      pendingUpdates.current[menuItemId]!.sequence > sequence
    ) {
  ...
  const handleIncrement = (menuItemId: string, price: number) => {
    setLocalQuantities((prev) => {
      const currentQty =
        prev[menuItemId] ?? quantityRefs.current[menuItemId] ?? 0;
        ...
        <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
          {items.map((item) => (
            <div
              ...
              className="p-3 bg-white rounded-lg border border-orange-100 space-y-2"
            >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          handleDecrement(item.menuItem.id, item.menuItem.price)
                        }
                        className="w-7 h-7 p-0 border-orange-200 hover:bg-orange-50"
                      >
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          handleIncrement(item.menuItem.id, item.menuItem.price)
                        }
                        className="w-7 h-7 p-0 border-orange-200 hover:bg-orange-50"
                      >
